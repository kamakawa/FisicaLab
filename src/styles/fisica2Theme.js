// src/styles/fisica2Theme.js
// Tema compartilhado dos experimentos de Física 2 — paleta vermelho-carmesim.
// Cada experimento importa FISICA2_BASE_STYLES e concatena seus próprios
// estilos específicos (ex: .canvas-wrap customizado, .plots-strip, etc.)
// Evita duplicar a paleta/estrutura em cada arquivo, como acontecia em Física 1.

// Constantes físicas compartilhadas (gás ideal diatômico, ex: ar — N2/O2).
// Mantidas num só lugar para não haver inconsistência entre experimentos
// (ex: usar Cv monoatômico com γ diatômico ao mesmo tempo).
export const GAS = {
  R: 0.0821,   // L·atm / (mol·K)
  GAMMA: 1.4,  // Cp/Cv — gás diatômico
  CV: 2.5,     // Cv = (5/2)R — gás diatômico
  CP: 3.5,     // Cp = (7/2)R — gás diatômico
};

export const FISICA2_BASE_STYLES = `
...
.canvas-wrap {
  flex: 1;
  position: relative;
  background: radial-gradient(ellipse at 50% 50%, #241014 0%, #0B0F19 100%);
  overflow: hidden;
}
...
.card:hover { border-color: rgba(239, 68, 68, 0.45); }
...
.ctrl-num  { font-family: var(--mono); color: var(--accent); background: rgba(239, 68, 68, 0.12); padding: 2px 8px; border-radius: 4px; }
input[type=range] {
  width: 100%;
...
.eq-block:hover { border-color: rgba(239, 68, 68, 0.45); }
.eq-block .eq-title {
  font-family: var(--sans);
  font-size: 11px;
  font-weight: 700;
...
.big-eq:hover { border-color: rgba(239, 68, 68, 0.45); }
